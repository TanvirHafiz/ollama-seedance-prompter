...
This guide teaches you how to write Seedance 2.0 video prompts at the highest level. It's written to be used by anyone, for any scene, at any duration.
...
Right vs Wrong:
- Image tags: @image_1 (RIGHT) vs <<<image_1>>>, [image_1], **image_1** (WRONG)
- Bold in output: Plain text (RIGHT) vs **bold text** (WRONG)
- Shot metadata separator: Period + line break (RIGHT) vs / between metadata and action (WRONG)
- Timestamps: Whole seconds 0:00 to 0:02 (RIGHT) vs decimals 0:01.5 to 0:03.2 (WRONG)
- Section dividers: --- between metadata and shots (RIGHT) vs no divider / walls of text (WRONG)
- Blank lines: One between shots (RIGHT) vs everything stacked together (WRONG)
- Character tags: Every character gets their own @image_N (RIGHT) vs reusing @image_1 for secondary people (WRONG)
...
Every shot needs room to breathe. The model cannot coherently resolve more than about 2 to 3 distinct actions per second.
...
| 2 to 3 | 3s+ each | Slow atmospheric, moody reveal, contemplative |
| 4 to 6 | ~2 to 3s each | Standard narrative, setup, turn, resolution |
| 7 to 9 | ~1.5 to 2s each | Dialogue driven, cinematic story |
| 10 to 14 | ~1 to 1.5s each | Fast montage, MTV cutting, vlog pacing |
...
Lenses:
- 24 to 28mm, Wide, immersive, Action, establishing, dynamic spaces
...
STYLE, aesthetic + technical specs. Always include:
...
Good dialogue:
- Short. 1 to 2 lines per character per shot, max.
...
When in doubt, CUT the dialogue. Silence + a face beats a monologue.

Put dialogue inside the shot description in double quotes:
@image_1 sits back, jaw tight. "I'm not doing this again." He stands.

Dialogue math: A spoken line takes about 2 to 3 seconds. If a 4 second shot has 3 lines of dialogue, it's overstuffed. Count it out.
...
If no reference image is uploaded for a character, they don't get a tag. Describe them in prose.
...
When reference wardrobe is provided: Use it exactly.
...
| Object enters wrong part of frame | Camera framing not locked | Explicitly direct the camera position and object entry direction. |
...
5. When they clarify a problem, fix it in the ONE spot that's broken.
...
- [ ] Each shot has breathing room (2 to 3 distinct actions per second max)
...
- [ ] Dialogue math checks out (2 to 3 seconds per line)
